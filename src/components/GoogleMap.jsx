...
import { default as initialGeoJson } from '../Geojson';
    ...
    return (
      ...
      >
        ...
        {features.reduce((array, feature, index) => {
          ...
          const { ElementClass, ChildElementClass, ...geometry }
            = geometryToComponentWithLatLng(feature.geometry);
          const { visible, child, ...featureState } = geoStateBy[feature.id] || {};

          if (visible !== false) {
            array.push(
              <ElementClass key={`json-${feature.id}`}
                {...properties} {...geometry} {...featureState}
              >
